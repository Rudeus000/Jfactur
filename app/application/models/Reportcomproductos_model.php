...
class Reportcomproductos_model extends CI_Model {

	function getreportproductos($data)
	{
		$this->db->from('tb_compra_detalle as tb');
		$this->db->select('tb.cod_producto, p.nomb_product, m.nomb_marca,ca.nomb_categoria, u.nomb_unid, SUM(tb.cant_compdet) as cantidad, SUM(tb.subtotal_compdet) as total');

		$this->db->join('tb_compra as c','tb.cod_comp = c.cod_comp');
        ...
        if (isset($data['desde']) AND isset($data['hasta'])) {
			$this->db->where('c.fecha_comp >=',$data['desde']);
			$this->db->where('c.fecha_comp <=',$data['hasta']);
		}

		if ($data['tb_producto']!='') {
        $this->db->like('nomb_product',$data['tb_producto']);
         }
