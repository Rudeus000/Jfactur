...
class Reportestock_model extends CI_Model {
    ...
    END as stock,
		...
		$this->db->join('tb_producto_stock','tb_compra.cod_producto = tb_producto_stock.cod_producto');
    ...
    $result['iTotalRecords'] = $queryLike->num_rows();
		...
		$row = [];
    foreach ($query->result() as $q) {
			$row[] = [$q->fecha_vent,$q->nomb_almacen,$q->nomb_puntoventa,$q->nomb_cliente,$q->nom_tipdocumento.'-'.$q->serie.'-'.$q->numero_vent,$q->nombre_apellido,$q->producto_ventdet,$q->serie_ventdetserie,$q->precunit_ventdet,$q->descuento_ventdet,$q->precunit_con_descuento,$q->cantidad,$q->subtotal];
		}
