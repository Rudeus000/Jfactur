...
class RegBusquedaGeneral extends CI_Controller {
    ...
    public function getProductoBusqueda()
    {
        ...
        $resultProducto = $this->db->from('tb_producto')
            ->select("tb_producto.cod_producto as id,nomb_product as nombre,(prec_costo / " . $cambio . ") as costo,(" . $precioVenta . " / " . $cambio . ") as venta,nomb_unid as unidad, (CASE WHEN stock > stockmin_product THEN 1 ELSE 0 END) as estado,peso_product, tb_producto.idTypeAssignmentProduct,stock,cod_tiparticulo, tb_almacen.nomb_almacen, tb_almacen.cod_almacen", FALSE)
        ...
        ->select("tb_producto.cod_producto as id,nomb_product as nombre,(prec_costo / ".$cambio.") as costo,(".$precioVenta." / ".$cambio.") as venta,nomb_unid as unidad, '1' as estado,peso_product,cod_tiparticulo, tb_almacen.nomb_almacen, tb_almacen.cod_almacen",FALSE)
        ->join('tb_unidades','tb_producto.cod_unid = tb_unidades.cod_unid')
        ->where_in('tb_producto.typeAssignmentProduct', array('H', 'N'))
        ->where('est_product',1)
    ...
    public function getProducto()
		{
			...
			$result = $this->db->from('tb_producto')
					->select("tb_producto.*,tb_marca.*,tb_unidades.*,tb_producto_stock.*,tb_categoria.*,tb_almacen.*,(prec_costo / " . $cambio . ") as costo,(prec_venta / " . $cambio . ") as venta,(tb_producto_stock.stock - stockmin_product ) as stock_disponible,cod_tiparticulo", FALSE)
					->join('tb_producto_stock', $JoinAssignmentProduct)
					->join('tb_almacen','tb_producto_stock.cod_almacen = tb_almacen.cod_almacen')
					->join('tb_marca', 'tb_producto.cod_marca = tb_marca.cod_marca')
					->join('tb_unidades', 'tb_producto.cod_unid = tb_unidades.cod_unid')
					->join('tb_categoria','tb_producto.cod_categoria = tb_categoria.cod_categoria')
