...
class Regreportventotal extends CI_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->load->model('reportventastotal_model');
	}
	...
	$data['usuario'] = $this->modelgeneral->getTableWhere('tb_usuario',['cod_grupo' => '6' ,'estado_usuario' =>'1']);

	//$data['talonario'] = $this->modelgeneral->getTableWhere('tb_talonario',['siglas_talonario'=>'PC']);	
	$this->load->view('layouts/header');
    $this->load->view('layouts/aside');
    $this->load->view('reports/reportventatotal',$data);    
    $this->load->view('layouts/footer');
	}

	public function jsonReportventotal()
	{
		$data['start'] = $this->input->get_post('start', true);
		$data['length'] = $this->input->get_post('length', true);
        $data['sEcho']  = $this->input->get_post('_', true);
        $columns= ['cod_vent','cod_vent'];
		...
		$correlativo = $this->input->get_post('correlativo');
		$cliente = $this->input->get_post('cliente');
		$punto = $this->input->get_post('punto');
		$vendedor = $this->input->get_post('vendedor');
		$pago_cot = $this->input->get_post('pago_cot');
		//$tb_talonario = $this->input->get_post('tb_talonario');
		$moneda_cat = $this->input->get_post('moneda_cat');
		...
		$data['pago_cot'] = $pago_cot;
		$data['moneda_cat'] = $moneda_cat;
		$data['estado'] = $estado;	
		$datos = $this->reportventastotal_model->getreportventastotal($data);
