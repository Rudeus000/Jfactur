	...
	public function query($almacen, $anio, $mes)
	{
		...
		$query = $this->db->from('tb_venta_detalle')
			->select(
				...
				SUM(CASE WHEN tipo_ventdet='O' THEN prec_ventdet ELSE 0 END) as obsequio_precio,
				SUM(CASE WHEN tipo_ventdet='B' THEN prec_ventdet ELSE 0 END) as bonificacion_precio",
				FALSE
			)
		...
		foreach ($query as $q) {
			...
			$valor=(($q->stock_inicial*$q->prec_costo)/1.18);
			$q->venta_costo=($valor+$q->compra_precio)/($q->stock_inicial+$q->stock);			
			$q->venta_costo =  round($q->venta_costo, 2);
			...
			$q->valorado_stock_inicial = $valor;
			$q->valorado_traspasos_recibidos = $q->traspasos_recibidos * $q->compra_precio;
			$q->valorado_traspasos_enviados = $q->traspasos_enviados * $q->compra_precio;
			$q->venta_precio=$q->venta_costo * $q->venta_cantidad;			

			$q->valorado_stock_final =  ($valor + $q->compra_precio + $q->valorado_traspasos_recibidos)-($q->venta_precio + $q->valorado_traspasos_enviados + $q->obsequio_precio + $q->bonificacion_precio);
			
			$q->venta_precio =  round($q->venta_precio, 2);

			// $q->valorado_stock_final =  ($q->stock_final * $q->venta_costo);


			$q->valorado_stock_inicial =  round($q->valorado_stock_inicial, 2);		
