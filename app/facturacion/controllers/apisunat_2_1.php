...
<cbc:PaymentDueDate>' . $value['fecha_ventcuo'] . '</cbc:PaymentDueDate>
        ...
        for ($i = 0; $i < count($cabecera['TIPO_IGV']); $i++) {
            ...
            if (!in_array($cabecera['TIPO_IGV'][$i], $tipos_igv_procesados)) {
                $cabSutotalArray[] = '<cac:TaxSubtotal>
<cbc:TaxableAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $cabecera["TOTAL_GRA_EXO_FRE"][$i] . '</cbc:TaxableAmount>
<cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $cabecera["TOTAL_IGV"][$i] . '</cbc:TaxAmount>
<cac:TaxCategory>
<cbc:ID schemeID="UN/ECE 5305" schemeName="Tax Category Identifier" schemeAgencyName="United Nations Economic Commission for Europe">' . $cabecera['TAX_CATEGORY_IDENTIFIER'][$i] . '</cbc:ID>
<cac:TaxScheme>
<cbc:ID schemeID="UN/ECE 5153" schemeAgencyID="6">' . $cabecera['TIPO_IGV'][$i] . '</cbc:ID>
<cbc:Name>' . $cabecera['IGV_EXO'][$i] . '</cbc:Name>
<cbc:TaxTypeCode>' . $cabecera['TIPO_IMPUESTO'][$i] . '</cbc:TaxTypeCode>
</cac:TaxScheme>
</cac:TaxCategory>
</cac:TaxSubtotal>';

                // Agregar el tipo de IGV actual al array de tipos procesados
                $tipos_igv_procesados[] = $cabecera['TIPO_IGV'][$i];
            }
        }

        // Concatenar los bloques en un solo string
        $cabSutotal = implode('', $cabSutotalArray);
...
<cbc:IdentificationCode listID="ISO 3166-1" listAgencyName="United Nations Economic Commission for Europe" listName="Country">' . $cabecera["COD_PAIS_CLIENTE"] . '</cbc:IdentificationCode>
	...
	<cac:TaxTotal>
<cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $cabecera["TOTAL_IGV_CABECERA"] . '</cbc:TaxAmount>' .
            $cabSutotal; //TOTAL=GRAVADA+IGV+EXONERADA
        //NO ENTRA GRATUITA(INAFECTA) NI DESCUENTO //SUB_TOTAL=PRECIO(SIN IGV) * CANTIDAD
        $xmlCPE = $xmlCPE .
            '</cac:TaxTotal>
...
<cbc:TaxAmount currencyID="' . $cabecera["COD_MONEDA"] . '">' . $detalle[$i]["txtIGV"] . '</cbc:TaxAmount>
<cac:TaxCategory>
<cbc:ID schemeID="UN/ECE 5305" schemeName="Tax Category Identifier" schemeAgencyName="United Nations Economic Commission for Europe">' . $detalle[$i]['TAX_CATEGORY_IDENTIFIER'] . '</cbc:ID>
...
<cbc:TaxTypeCode>' . $detalle[$i]['TIPO_IMPUESTO'] . '</cbc:TaxTypeCode>
</cac:TaxScheme>
</cac:TaxCategory>
</cac:TaxSubtotal>
</cac:TaxTotal>
...
<cbc:TaxTypeCode>OTH</cbc:TaxTypeCode>
    ...
    { //=================ZIPEAR ================
        ...
        $soapUrl = $ruta_ws;
        // xml post structure
        $xml_post_string = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ser="http://service.sunat.gob.pe" xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd">
<soapenv:Header>
<wsse:Security>
<wsse:UsernameToken>
...
<contentFile>' . base64_encode(file_get_contents($ruta_archivo . '.ZIP')) . '</contentFile>
        ...
        if ($httpcode == 200) {
            $doc = new DOMDocument();
            $doc->loadXML($response); //===================VERIFICAMOS SI HA ENVIADO CORRECTAMENTE EL COMPROBANTE=====================
            if (isset($doc->getElementsByTagName('applicationResponse')->item(0)->nodeValue)) {
                ...
                unlink($ruta_archivo_cdr . 'R-' . $archivo . '.ZIP'); //=============hash CDR=================
