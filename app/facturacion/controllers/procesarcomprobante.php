...
class Procesarcomprobante {
	...
	public function procesarXmlFactura($data_comprobante, $items_detalle, $rutas)
	{
		$apisunat = new apisunat();
		$resp = $apisunat->crear_xml_factura($data_comprobante, $items_detalle, $rutas['ruta_xml']);
		$signature = new Signature();
	...
	public function procesarEnviarFactura($data_comprobante,$rutas)
	{
		$apisunat = new apisunat();
		$resp_envio = $apisunat->enviar_documento($data_comprobante['EMISOR_RUC'], $data_comprobante['EMISOR_USUARIO_SOL'], $data_comprobante['EMISOR_PASS_SOL'], $rutas['ruta_xml'], $rutas['ruta_cdr'], $rutas['nombre_archivo'], $rutas['ruta_ws']);
	...
	public function ConsultarEstadoTicket($data_comprobante, $rutas,$ticket) {
		$apisunat = new apisunat();
		//var_export($data_comprobante);
		$resp_ticket = $apisunat->consultar_envio_ticket($data_comprobante['ruc'], $data_comprobante['usuariosol'], $data_comprobante['clavesol'], $ticket, $rutas['nombre_archivo'], $rutas['ruta_cdr'], $rutas['ruta_ws']);		
		return $resp_ticket;
	}
}
?>