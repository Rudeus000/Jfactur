<?php
	...
	//RECORDAR QUE NO SE HA ACTUALIZADO LA DEMO, POR LO TANTO EL DETALLE QUE INGRESE POR AQUÍ LE FALTA EL ITEM
	//"txtCODIGO_PROD_SUNAT"

	//var_dump($items_detalle);
	//exit();

	$ruc_emisor = "20574694044";
    $emisor['ruc'] = $ruc_emisor;
    $emisor['tipo_doc'] = "6";
    $emisor['nom_comercial'] = "";
    $emisor['razon_social'] = 'MOBILE TECHNOLOGY SOLUTIONS S.A.C';
    $emisor['codigo_ubigeo'] = "050101";
    $emisor['direccion'] = "AV. A.A. CACERES NRO. 872";
    $emisor['direccion_departamento'] = 'AYACUCHO';
    $emisor['direccion_provincia'] = 'HUAMANGA';
    $emisor['direccion_distrito'] = 'AYACUCHO';
    $emisor['direccion_codigopais'] = 'PE';
    $emisor['usuariosol'] = 'UREXPORD';
    $emisor['clavesol'] = 'houritige';
    $tipodeproceso = '1'; //3=beta,2=homologacion,1=produccion
    ...
    if ($tipodeproceso == '1') {
        ...
        if (file_exists('produccion/' . $ruc_emisor . '.p12')) {
            $ruta_firma = $url_base . $content_firmas.'produccion/' . $ruc_emisor . '.p12';
        } else {
            $ruta_firma = $url_base . $content_firmas.'produccion/certificado.p12';
            $pass_firma = 'MTS22092020';
        }        
	...
	if($tipo_comprobante == "01") {
		...
		$resp['ruta_cdr'] = 'archivos_xml_sunat/cpe_xml/beta/20574694044/R-'.$archivo.'.XML';
