...
$resp = $procesarcomprobante->procesarXmlFactura($array_cabecera, $array_detalle, $rutas);
...
function get_array_cabecera($data, $emisor)
{
	$cabecera = array(
		...
		'TOTAL_GRATUITAS' => (isset($data['total_free'])) ? $data['total_free'] : "0",
		...
		'TOTAL_IGV_CABECERA' => (isset($data['total_igv'])) ? $data['total_igv'] : "0",
		...
		'IGV_EXO' => $data['IGV_EXO'],
		'TIPO_IMPUESTO' => $data['TIPO_IMPUESTO'],
		'TAX_CATEGORY_IDENTIFIER' => $data['TAX_CATEGORY_IDENTIFIER'],
		
		'IGV_EXO_VAT' => $data['IGV_EXO_VAT'],
		'IGV_EXO_EXO' => $data['IGV_EXO_EXO'],
		'IGV_EXO_FREE' => $data['IGV_EXO_FREE'],		
		'TOTAL_GRA_EXO_FRE' => $data['TOTAL_GRA_EXO_FRE'],
		...
		'CONTACTO_EMPRESA' => "",
		'CODIGO_SUNAT' => $emisor['codigo_sunat'],
